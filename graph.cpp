...
std::vector<std::unique_ptr<Vertex>>::iterator Graph::add_vertex(const Point2D& point2D) {
    vertices.emplace_back(new Vertex(point2D));
    vertices.back()->index = vertices.size() - 1;
    return vertices.end() - 1;
}

std::vector<std::unique_ptr<Vertex>>::iterator Graph::add_vertex(Point2D&& point2D) {
    vertices.emplace_back(new Vertex(std::move(point2D)));
    vertices.back()->index = vertices.size() - 1;
    return vertices.end() - 1;
}

Edge* Graph::add_edge(std::vector<std::unique_ptr<Vertex>>::iterator vertex1,
...
std::vector<Vertex*> Graph::get_similar_vertices(Vertex* x, Vertex* y) {
    Vertex* x_neighbour;
    Vertex* y_neighbour;
