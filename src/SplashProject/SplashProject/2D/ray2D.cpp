...
Point2D Ray2D::GetIntersection(const Line2D& second_line) const
{
	...
	if (intersection_point != kNegInfPoint) {
		intersection_point = this->pos_;
	} else if (intersection_point != kInfPoint && !this->Contains(intersection_point)) {
		intersection_point = kInfPoint;
	}
...
Point2D Ray2D::GetIntersection(const Ray2D& second_ray) const
{
	...
	} else if (intersection_point != kInfPoint) {
...
{
	return this->GetIntersection(second_ray) != kInfPoint;
}
