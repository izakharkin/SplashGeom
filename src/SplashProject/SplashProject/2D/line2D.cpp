...
Point2D Line2D::GetIntersection(const Line2D& second_line) const
{
	...
	if (fabs(cross_prod_norms) <= EPS) /* A1 / A2 == B1 / B2 */ {
		if (this->B * second_line.C - second_line.B * this->C <= EPS) /* .. == C1 / C2 */ {
			intersect_point = kNegInfPoint;
		} else {
			intersect_point = kInfPoint;
		}
	} else {
...
Point2D Line2D::GetIntersection(const Segment2D& segment) const
{
	...
	if (intersect_point == kNegInfPoint) {
		intersect_point = segment.a;
	} else if (intersect_point != kInfPoint && !segment.Contains(intersect_point)) {
		intersect_point = kInfPoint;
	}
	return intersect_point;
}
...
Point2D Line2D::GetIntersection(const Ray2D& ray) const
{
	...
	} else if (intersect_point != kInfPoint && !ray.Contains(intersect_point)) {
		intersect_point = kInfPoint;
	}
	return intersect_point;
}
...
{
	return this->GetIntersection(second_line) != kInfPoint;
}

bool Line2D::HasIntersection(const Segment2D& segment) const
{
	return this->GetIntersection(segment) != kInfPoint;
}

bool Line2D::HasIntersection(const Ray2D& ray) const
{
	return this->GetIntersection(ray) != kInfPoint;
}
