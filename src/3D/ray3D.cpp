...
Point3D Ray3D::GetIntersection(const Line3D& second_line) const
{
	...
	if (intersect_point != Point3D(INF, INF) && !this->Contains(intersect_point))
	{
...
Point3D Ray3D::GetIntersection(const Ray3D& second_ray) const
{
	...
	else if (intersect_point != Point3D(INF, INF))
	{
...
{
	return this->GetIntersection(second_ray) != Point3D(INF, INF);
}
