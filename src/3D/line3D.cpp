...
{
	double cross_prod_norms = Vector3D(this->A, this->B).OrientedCCW(Vector3D(second_line.A, second_line.B));
	Point3D intersect_point;

	if (cross_prod_norms <= EPS) // A1 / A2 == B1 / B2
	{
		if (this->B * second_line.C == second_line.B * this->C) // .. == C1 / C2
			intersect_point = Point3D(-INF, -INF);
		else
			intersect_point = Point3D(INF, INF);
	}
	else
	{
		...
		intersect_point = Point3D(res_x, res_y);
	}
...
Point3D Line3D::GetIntersection(const Segment3D& segment) const
{
	...
	if (intersect_point == Point3D(-INF, -INF))
	{
		intersect_point = segment.a;
	}
	else if (intersect_point != Point3D(INF, INF) && !segment.Contains(intersect_point))
	{
		intersect_point = Point3D(INF, INF);
	}
...
Point3D Line3D::GetIntersection(const Ray3D& ray) const
{
	...
	else if (intersect_point != Point3D(INF, INF) && !ray.Contains(intersect_point))
	{
		intersect_point = Point3D(INF, INF);
	}
...
{
	return this->GetIntersection(second_line) != Point3D(INF, INF);
}

bool Line3D::HasIntersection(const Segment3D& segment) const
{
	return this->GetIntersection(segment) != Point3D(INF, INF);
}

bool Line3D::HasIntersection(const Ray3D& ray) const
{
	return this->GetIntersection(ray) != Point3D(INF, INF);
}
